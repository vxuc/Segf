...
namespace UnitTest1
{
	...
	public:
		...
		TEST_METHOD(PrototypeTest1)
		{
			...
			//using the do_action function and passing in the choice parameter
			//and using the Iam function to return a string
			Action* action = Factory::do_action(choice);
			string iAm = action->Iam();

			//using the get_prototype function and passing in the same choice parameter
			//and using the Iam function to return a string
